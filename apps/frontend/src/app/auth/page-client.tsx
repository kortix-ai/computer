...
const OTP_SLOT_IDS = ['otp-slot-0', 'otp-slot-1', 'otp-slot-2', 'otp-slot-3', 'otp-slot-4', 'otp-slot-5'] as const;
...
function LoginContent({
  ...
  const handleLockScreenKeyDown = useCallback(
  ...
  useEffect(() => {
    if (linkExpired && newCodeSent) {
      otpInputRef.current?.focus();
    }
  }, [linkExpired, newCodeSent]);
              ...
              <div className="flex flex-col items-center gap-6 w-full max-w-[340px]">
                ...
                <label htmlFor="otp-input" className="w-full cursor-text">
                  ...
                  <input
                    ...
                    className="sr-only" id="otp-input"
                  />
                </label>
    ...
    <LazyMotion features={domAnimation}>
      <div
        className={cn(
          ...
          phase === 'lock' && 'cursor-pointer',
        )}
        role={phase === 'lock' ? 'button' : undefined}
        tabIndex={phase === 'lock' ? 0 : -1}
        aria-label={phase === 'lock' ? 'Open sign in form' : undefined}
        onClick={phase === 'lock' ? () => setPhase('form') : undefined}
        onKeyDown={phase === 'lock' ? handleLockScreenKeyDown : undefined}
      >
...
function SelfHostedLoginContent({ returnUrl, redirectParam }: AuthPageClientProps) {
  ...
  const [validatedRedirectPath, setValidatedRedirectPath] = useState<string | null>(null);
  ...
  const shouldAutoRedirect = !isLoading && !!user && installed !== false && wizardStepRef.current <= 1;

  useEffect(() => {
    if (!shouldAutoRedirect || !redirectParam) return;
    ...
    supabase.auth.getUser().then(({ error }) => {
      ...
      setValidatedRedirectPath(returnUrl || '/onboarding');
    });

    return () => {
      cancelled = true;
    };
  }, [redirectParam, returnUrl, shouldAutoRedirect, supabase, wizardStep]);
  ...
  if (shouldAutoRedirect && !redirectParam) {
    redirect(returnUrl || '/onboarding');
  }

  if (validatedRedirectPath) {
    redirect(validatedRedirectPath);
  }

  if (isLoading || validatingSession || statusLoading || (shouldAutoRedirect && !!redirectParam && !validatedRedirectPath)) {
    return (
      <div className="fixed inset-0 bg-background flex items-center justify-center">
        <KortixLoader size="medium" />
      </div>
